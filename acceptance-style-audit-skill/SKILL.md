...
description: 对比开发实现 JSON 与设计稿 JSON（或样式信息），仅聚焦颜色、字体、圆角、投影、间距等样式差异，忽略文案差异，输出结构化验收报告表格（问题位置、问题描述、修改建议、优先级、备注）。当用户提到样式验收、还原度对比、设计走查、UI QA、生成验收表时使用。
---

# Acceptance Style Audit

用于 UI 样式验收的专用 skill：输入「开发实现 JSON」与「设计稿 JSON/样式信息」，自动产出仅包含样式差异的验收报告。
